...
public class FileRenameCommandParserLink : CommandParserLinkBase
{
    private readonly IParameterHandler _parameterChain;

    public FileRenameCommandParserLink(IParameterHandler parameterChain)
    {
    ...
    public override ParserResult Parse(TokenIterator input)
    {
        ...
        if (command == null || !command.Equals("Rename", StringComparison.OrdinalIgnoreCase))
        {
        ...
        if (builder.Path != null && builder.NewName != null)
        {
